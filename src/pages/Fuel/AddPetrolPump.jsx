...
const AddPetrolPump = () => {
    ...
    const handleSave = async () => {
        if (!formData.name) {
            toast.error(t('please_enter_pump_name'));
            return;
        }

        setLoading(true);
        try {
            await petrolPumpService.addPetrolPump(formData);
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', height: '100%', background: 'var(--color-bg-body)' }}>
            <div style={{ padding: '1.25rem 1.5rem', display: 'flex', alignItems: 'center', gap: '1rem', background: 'var(--color-bg-surface)', borderBottom: '1px solid var(--color-border)' }}>
                ...
                <h2 style={{ fontSize: '1.5rem', fontWeight: 900, color: 'var(--color-text-main)', textTransform: 'uppercase' }}>{t('add_pump')}</h2>
            </div>

            <main style={{ flex: 1, padding: '1.5rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
                <Input label={t('pump_name')} name="name" placeholder={t('pump_name_placeholder')} value={formData.name} onChange={handleChange} />
                <Input label={`${t('location')} (${t('optional')})`} name="location" placeholder={t('location_placeholder')} value={formData.location} onChange={handleChange} />
